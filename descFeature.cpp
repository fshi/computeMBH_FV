...
#define epislon 0.000001  //epison 
...
void opcvL1Norm(float *arr, int sz, bool useThres = 0) 
{
	Mat m1(1, sz, CV_32FC1, arr);
	Mat m2(1, sz, CV_32FC1);
	cv::max(m1, 0., m2); //take value large than zero
	
	normalize(m2, m2, 1.0, 0., NORM_L1);
	m2.copyTo(m1);
}

void opcvSqrt(float *arr, int sz) 
{
	Mat m1(1, sz, CV_32FC1, arr);
	cv::sqrt(m1, m1);
}

void opcvL1NormSqrt(float *arr, int sz) 
{
	Mat m1(1, sz, CV_32FC1, arr);
	Mat m2(1, sz, CV_32FC1);
	cv::max(m1, 0., m2); //take values large than zero
	normalize(m2, m2, 1.0, 0.0, NORM_L1);
	cv::sqrt(m2, m2);
...
//compute features for parts' windows corresponding to root's window (tlp0, whl0)
	...
	{
		opcvL2Norm(ft+i, _rtSz);  //normalize 
		//l2Norm(ft+i, _rtSz, 0.2, 1);

	}
	
/*
	opcvL2Norm(ft, _rtSz); 
	opcvL2Norm(ft+_feaSz/2, _rtSz); 
*/	
	opcvL2Norm(ft+_rtSz, _feaSz/2-_rtSz);
	opcvL2Norm(ft+_rtSz+_feaSz/2, _feaSz/2-_rtSz);
	
/*
	opcvL2Norm(ft, _feaSz/2);
	//opcvL1NormSqrt(ft, _feaSz/2); 
	ft = outArr+_feaSz/2;
	opcvL2Norm(ft, _feaSz/2);
	//opcvL1NormSqrt(ft, _feaSz/2); 
	*/
